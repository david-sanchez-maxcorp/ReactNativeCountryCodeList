...
class CountryCodeList extends Component {
    ...
    return (
      <View style={styles.container}>
        <Search
          ...
          onChangeText={
            this.props.onSearch ? this.props.onSearch : this.onSearch
          }
      ...
      Object.keys(data).map(key => {
        data[key] = data[key].filter(el => {
          return (
            ...
            el.code.includes(this.state.query)
          );
        });
    ...
    return (
      <View
        style={[
          ...
          {
            backgroundColor: this.props.headerBackground,
            height: this.props.sectionHeaderHeight - 1
          }
        ]}
      >
        <Text
  ...
  renderSectionItem(rowData) {
    ...
    return (
      <Text style={[styles.sectionItemText, this.props.sectionItemTextStyle]}>
        {rowData.title}
      </Text>
    );
  }
      ...
      <View>
        <TouchableOpacity
          ...
          style={[
            ...
            { height: this.props.cellHeight - 0.5 }
          ]}
        >
          <Text
            numberOfLines={1}
            style={[styles.cellTitle, this.props.cellTitleStyle]}
          >
            {rowData.item.name}
          </Text>
          <Text style={[styles.cellLabel, this.props.cellLabelStyle]}>
            {rowData.item.code}
          </Text>
        </TouchableOpacity>
